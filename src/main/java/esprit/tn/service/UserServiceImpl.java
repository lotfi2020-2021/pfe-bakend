...
public class UserServiceImpl implements UserService {
    ...
    public User createNewUser(SignupDto signupDto) {
        try {
            ...
            newUser.setEntrepriseVerified(false);
...
//            emailService.send(savedUser.getEmail() , "create entreprise" , message);
            forgotPassword(savedUser.getEmail());
            return savedUser;
        }
        return null;
    }
...
//        }
    ...
    @Override
    public User updateAccountEntreprise(Long id1, Long id2) {

        User user =userRepository.getById(id1) ;
        User entreprise =userRepository.getById(id2) ;
        user.setEntrepriseVerified(true) ;
        user.setDemandeEntrepriseId(id2);
        String emailVerifyMail = "you are accepted to join Company " + entreprise.getFirstName() + " "+ entreprise.getLastName()  +", Welcome!" ;
        String emailVerifyMail2 = "a new demande sended to join your company" ;
        emailService.send(user.getEmail(), "Company Accept", emailVerifyMail );
        emailService.send(entreprise.getEmail(), "New demande company", emailVerifyMail );
        return user;
    }
    ...
    public User resetPassword(String token, ResetPasswordDto resetPasswordDto) {
     ...
     @Override
       public List<User> getDemandeUsers(Long id){
       List<User> list = userRepository.getDemandesUsersByentrepriseId(id) ;
        return list ;
    }
