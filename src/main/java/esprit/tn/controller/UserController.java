...
//
    ...
    @PutMapping ("/account/update/entreprise/{id}/{entrepriseId}")
    public ResponseEntity<?> updateUserEntreprise( @PathVariable  Long entrepriseId , @PathVariable("id") Long id ) {
        User updatedUser = userService.updateAccountEntreprise(id , entrepriseId);
    ...
    @GetMapping("/demandeUsers/{id}")
    public ResponseEntity<?> getDemandeUsers( @PathVariable Long id) {
        List<User> list = userService.getDemandeUsers(id);
        List<UserDto> listDtos =userMapper.usersToUsersDTO(list) ;
        return new ResponseEntity<>(listDtos, HttpStatus.OK);
    }



    @PutMapping ("/assignUserToEntreprise/{id}")
    public ResponseEntity<?> assignUserToEntreprise(@PathVariable Long id) {
        User user = userRepository.findById(id).get();
        User entreprise =userRepository.getById(user.getDemandeEntrepriseId());
        user.setEntrepriseVerified(false);
            user.setEntreprise(entreprise);
         userRepository.save(user) ;
        return new  ResponseEntity(HttpStatus.OK);
    }

    @PutMapping ("/annulerAsignUserToEntreprise/{id}")
    public ResponseEntity<?> annulerAsignUserToEntreprise(  @PathVariable Long id) {
        User user = userRepository.findById(id).get();

        user.setEntreprise(null);
        user.setEntrepriseVerified(false);
        user.setDemandeEntrepriseId(null);
        userRepository.save(user);
        return new  ResponseEntity(HttpStatus.OK);
    }
